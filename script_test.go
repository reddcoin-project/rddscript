...
func TestPushedData(t *testing.T) {
	var tests = []struct {
		...
		{
			[]byte{rddscript.OP_0, rddscript.OP_IF, rddscript.OP_0, rddscript.OP_ELSE, rddscript.OP_2, rddscript.OP_ENDIF},
			[][]byte{{}, {}},
			true,
		},
		{
			rddscript.NewScriptBuilder().AddInt64(16777216).AddInt64(10000000).Script(),
			[][]byte{
		...
		{
			rddscript.NewScriptBuilder().AddOp(rddscript.OP_DUP).AddOp(rddscript.OP_HASH160).
				AddData([]byte("17VZNX1SN5NtKa8UQFxwQbFeFc3iqRYhem")).AddOp(rddscript.OP_EQUALVERIFY).
				AddOp(rddscript.OP_CHECKSIG).Script(),
			[][]byte{
		...
		{
			rddscript.NewScriptBuilder().AddOp(rddscript.OP_PUSHDATA4).AddInt64(1000).
				AddOp(rddscript.OP_EQUAL).Script(),
			[][]byte{},
			false,
		},
	}
	...
	for x, test := range tests {
		pushedData, err := rddscript.PushedData(test.in)
...
func TestStandardPushes(t *testing.T) {
	for i := 0; i < 1000; i++ {
		...
		if result := rddscript.IsPushOnlyScript(builder.Script()); !result {
			t.Errorf("StandardPushesTests IsPushOnlyScript test #%d failed: %x\n", i, builder.Script())
		}
		if result := rddscript.HasCanonicalPushes(builder.Script()); !result {
...
type txTest struct {
	...
	scriptInfo    rddscript.ScriptInfo // result of ScriptInfo
...
var txTests = []txTest{
	...
	// blockchain that verifies signatures.
	{
		...
		pkScript: []byte{
			...
			0x12, 0xa3, rddscript.OP_CHECKSIG,
		},
		idx:     0,
		nSigOps: 1,
		scriptInfo: rddscript.ScriptInfo{
			PkScriptClass:  rddscript.PubKeyTy,
			NumInputs:      1,
			ExpectedInputs: 1,
			SigOps:         1,
		},
	...
	// Previous test with the value of one output changed.
	{
		...
		idx:     0,
		err:     rddscript.ErrStackScriptFailed,
		nSigOps: 1,
		scriptInfo: rddscript.ScriptInfo{
			PkScriptClass:  rddscript.PubKeyTy,
			NumInputs:      1,
	...
	// uses checksig with SigHashNone.
	{
		...
		nSigOps: 1,
		scriptInfo: rddscript.ScriptInfo{
			PkScriptClass:  rddscript.PubKeyHashTy,
			NumInputs:      2,
			ExpectedInputs: 2,
			SigOps:         1,
		},
	...
	// Uses OP_CODESEPARATOR and OP_CHECKMULTISIG
	{
		...
		pkScript: []byte{
			...
			0x4f, 0x13,
			rddscript.OP_NOP2, rddscript.OP_DROP,
		},
		idx:           1,
		bip16:         false,
		nSigOps:       0, // multisig is in the pkScript!
		scriptInfoErr: rddscript.ErrStackNonPushOnly,
	},
	...
	// multisig with zero required signatures
	{
		...
		nSigOps: 1,
		scriptInfo: rddscript.ScriptInfo{
			PkScriptClass:  rddscript.ScriptHashTy,
			NumInputs:      2,
			ExpectedInputs: 2,
			SigOps:         1,
		},
	...
	},
	{
		...
		idx:           0,
		err:           rddscript.ErrStackShortScript,
		bip16:         true,
		scriptInfoErr: rddscript.ErrStackShortScript,
	},
	{
		...
		idx:           0,
		parseErr:      rddscript.ErrStackP2SHNonPushOnly,
		bip16:         true,
		nSigOps:       0, // no signature ops in the pushed script.
		scriptInfoErr: rddscript.ErrStackNonPushOnly,
	},
	{
		...
		nSigOps:  0, // no signature ops in the pushed script.
		scriptInfo: rddscript.ScriptInfo{
			PkScriptClass:  rddscript.NonStandardTy,
			NumInputs:      1,
			ExpectedInputs: -1,
			SigOps:         0,
		},
...
func testTx(t *testing.T, test txTest) {
	...
	if test.canonicalSigs {
		flags |= rddscript.ScriptCanonicalSignatures
	}
	engine, err := rddscript.NewScript(
...
func TestGetPreciseSignOps(t *testing.T) {
	...
	for _, test := range txTests {
		count := rddscript.GetPreciseSigOpCount(
			test.tx.TxIn[test.idx].SignatureScript, test.pkScript,
			test.bip16)
	...
	psocTests := []psocTest{
		{
			...
			scriptSig: []byte{rddscript.OP_PUSHDATA1, 2},
			err:       rddscript.ErrStackShortScript,
		},
		{
			name:      "scriptSig isn't push only",
			scriptSig: []byte{rddscript.OP_1, rddscript.OP_DUP},
			nSigOps:   0,
		},
		{
		...
		},
		{
			...
			scriptSig: []byte{rddscript.OP_1, rddscript.OP_1},
			nSigOps:   0,
		},
		// pushed script doesn't parse.
		{
			name: "pushed script doesn't parse",
			scriptSig: []byte{rddscript.OP_DATA_2,
				rddscript.OP_PUSHDATA1, 2},
			err: rddscript.ErrStackShortScript,
		},
	}
...
func TestScriptInfo(t *testing.T) {
	for _, test := range txTests {
		si, err := rddscript.CalcScriptInfo(
	...
	extraTests := []scriptInfoTest{
		{
			...
			sigScript: []byte{rddscript.OP_TRUE,
				rddscript.OP_DATA_1, 81,
				rddscript.OP_DATA_8,
				rddscript.OP_2DUP, rddscript.OP_EQUAL,
				rddscript.OP_NOT, rddscript.OP_VERIFY,
				rddscript.OP_ABS, rddscript.OP_SWAP,
				rddscript.OP_ABS, rddscript.OP_EQUAL,
			},
			// truncated version of test below:
			pkScript: []byte{rddscript.OP_HASH160,
				rddscript.OP_DATA_20,
		...
		},
		{
			...
			pkScript: []byte{rddscript.OP_HASH160,
				...
				0xfe, 0x44, 0x10, 0x65, 0xb6, 0x53, 0x22, 0x31,
				0xde, 0x2f, 0xac, 0x56, 0x31, 0x52, 0x20, 0x5e,
				0xc4, 0xf5, 0x9c, 0x74, rddscript.OP_EQUAL,
			},
			bip16:         true,
			scriptInfoErr: rddscript.ErrStackShortScript,
		},
		{
			...
			sigScript: []byte{rddscript.OP_TRUE,
				rddscript.OP_DATA_1, 81,
				rddscript.OP_DATA_25,
				rddscript.OP_DUP, rddscript.OP_HASH160,
				rddscript.OP_DATA_20, 0x1, 0x2, 0x3, 0x4, 0x5,
				0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe,
				0xf, 0x10, 0x11, 0x12, 0x13, 0x14,
				rddscript.OP_EQUALVERIFY, rddscript.OP_CHECKSIG,
			},
			pkScript: []byte{rddscript.OP_HASH160,
				rddscript.OP_DATA_20,
				0xfe, 0x44, 0x10, 0x65, 0xb6, 0x53, 0x22, 0x31,
		...
		},
		{
			...
			sigScript: []byte{rddscript.OP_TRUE,
				rddscript.OP_TRUE, rddscript.OP_TRUE,
				rddscript.OP_0, // Extra arg for OP_CHECKMULTISIG bug
			},
			pkScript: []byte{
				rddscript.OP_3, rddscript.OP_DATA_33,
				...
				0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a,
				0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21,
				rddscript.OP_3, rddscript.OP_CHECKMULTISIG,
			},
			bip16: true,
			scriptInfo: rddscript.ScriptInfo{
				PkScriptClass:  rddscript.MultiSigTy,
				NumInputs:      4,
				ExpectedInputs: 4,
				SigOps:         3,
			},
	...
	for _, test := range extraTests {
		si, err := rddscript.CalcScriptInfo(test.sigScript,
			test.pkScript, test.bip16)
		if err != nil {
			if err != test.scriptInfoErr {
				t.Errorf("scriptinfo test \"%s\": got \"%v\""+
...
var removeOpcodeTests = []removeOpcodeTest{
	// Nothing to remove.
	{
		...
		before: []byte{rddscript.OP_NOP},
		remove: rddscript.OP_CODESEPARATOR,
		after:  []byte{rddscript.OP_NOP},
	},
	// Test basic opcode removal
	{
		name: "codeseparator 1",
		before: []byte{rddscript.OP_NOP, rddscript.OP_CODESEPARATOR,
			rddscript.OP_TRUE},
		remove: rddscript.OP_CODESEPARATOR,
		after:  []byte{rddscript.OP_NOP, rddscript.OP_TRUE},
	},
	// The opcode in question is actually part of the data in a previous
	// opcode
	{
		name: "codeseparator by coincidence",
		before: []byte{rddscript.OP_NOP, rddscript.OP_DATA_1, rddscript.OP_CODESEPARATOR,
			rddscript.OP_TRUE},
		remove: rddscript.OP_CODESEPARATOR,
		after: []byte{rddscript.OP_NOP, rddscript.OP_DATA_1, rddscript.OP_CODESEPARATOR,
			rddscript.OP_TRUE},
	},
	{
		name:   "invalid opcode",
		before: []byte{rddscript.OP_UNKNOWN186},
		remove: rddscript.OP_CODESEPARATOR,
		after:  []byte{rddscript.OP_UNKNOWN186},
	},
	{
		name:   "invalid length (insruction)",
		before: []byte{rddscript.OP_PUSHDATA1},
		remove: rddscript.OP_CODESEPARATOR,
		err:    rddscript.ErrStackShortScript,
	},
	{
		name:   "invalid length (data)",
		before: []byte{rddscript.OP_PUSHDATA1, 255, 254},
		remove: rddscript.OP_CODESEPARATOR,
		err:    rddscript.ErrStackShortScript,
	},
}
...
func testRemoveOpcode(t *testing.T, test *removeOpcodeTest) {
	result, err := rddscript.TstRemoveOpcode(test.before, test.remove)
	if test.err != nil {
		if err != test.err {
...
var removeOpcodeByDataTests = []removeOpcodeByDataTest{
	...
	{
		name:   "simple case",
		before: []byte{rddscript.OP_DATA_4, 1, 2, 3, 4},
		remove: []byte{1, 2, 3, 4},
		after:  []byte{},
	},
	{
		...
		before: []byte{rddscript.OP_DATA_4, 1, 2, 3, 4},
		remove: []byte{1, 2, 3, 5},
		after:  []byte{rddscript.OP_DATA_4, 1, 2, 3, 4},
	},
	{
		// padded to keep it canonical.
		name: "simple case (pushdata1)",
		before: append(append([]byte{rddscript.OP_PUSHDATA1, 76},
	...
	},
	{
		...
		before: append(append([]byte{rddscript.OP_PUSHDATA1, 76},
			bytes.Repeat([]byte{0}, 72)...), []byte{1, 2, 3, 4}...),
		remove: []byte{1, 2, 3, 5},
		after: append(append([]byte{rddscript.OP_PUSHDATA1, 76},
			bytes.Repeat([]byte{0}, 72)...), []byte{1, 2, 3, 4}...),
	},
	{
		name:   "simple case (pushdata1 miss noncanonical)",
		before: []byte{rddscript.OP_PUSHDATA1, 4, 1, 2, 3, 4},
		remove: []byte{1, 2, 3, 4},
		after:  []byte{rddscript.OP_PUSHDATA1, 4, 1, 2, 3, 4},
	},
	{
		name: "simple case (pushdata2)",
		before: append(append([]byte{rddscript.OP_PUSHDATA2, 0, 1},
	...
	},
	{
		...
		after: append(append([]byte{rddscript.OP_PUSHDATA2, 0, 1},
			bytes.Repeat([]byte{0}, 252)...), []byte{1, 2, 3, 4}...),
	},
	{
		name:   "simple case (pushdata2 miss noncanonical)",
		before: []byte{rddscript.OP_PUSHDATA2, 4, 0, 1, 2, 3, 4},
		remove: []byte{1, 2, 3, 4},
		after:  []byte{rddscript.OP_PUSHDATA2, 4, 0, 1, 2, 3, 4},
	},
	{
		// This is padded to make the push canonical.
		name: "simple case (pushdata4)",
		before: append(append([]byte{rddscript.OP_PUSHDATA4, 0, 0, 1, 0},
	...
	},
	{
		...
		before: []byte{rddscript.OP_PUSHDATA4, 4, 0, 0, 0, 1, 2, 3, 4},
		remove: []byte{1, 2, 3, 4},
		after:  []byte{rddscript.OP_PUSHDATA4, 4, 0, 0, 0, 1, 2, 3, 4},
	},
	{
		// This is padded to make the push canonical.
		...
		before: append(append([]byte{rddscript.OP_PUSHDATA4, 0, 0, 1, 0},
			bytes.Repeat([]byte{0}, 65532)...), []byte{1, 2, 3, 4}...),
		remove: []byte{1, 2, 3, 4, 5},
		after: append(append([]byte{rddscript.OP_PUSHDATA4, 0, 0, 1, 0},
			bytes.Repeat([]byte{0}, 65532)...), []byte{1, 2, 3, 4}...),
	},
	{
		name:   "invalid opcode ",
		before: []byte{rddscript.OP_UNKNOWN187},
		remove: []byte{1, 2, 3, 4},
		after:  []byte{rddscript.OP_UNKNOWN187},
	},
	{
...
func testRemoveOpcodeByData(t *testing.T, test *removeOpcodeByDataTest) {
	result, err := rddscript.TstRemoveOpcodeByData(test.before,
		test.remove)
	if test.err != nil {
		if err != test.err {
			t.Errorf("%s: got unexpected error. exp: \"%v\" "+
...
type scriptTypeTest struct {
	name       string
	script     []byte
	scripttype rddscript.ScriptClass
}
...
var scriptTypeTests = []scriptTypeTest{
	...
	// tx 599e47a8114fe098103663029548811d2651991b62397e057f0c863c2bc9f9ea
	{
		...
		scripttype: rddscript.PubKeyHashTy,
	},
	// part of tx 6d36bc17e947ce00bb6f12f8e7a56a1585c5a36188ffa2b05e10b4743273a74b
	// codeseparator parts have been elided. (Reddcoind's checks for multisig
	// type doesn't have codesep etc either.
	{
		...
		scripttype: rddscript.MultiSigTy,
	},
	// tx e5779b9e78f9650debc2893fd9636d827b26b4ddfa6a8172fe8708c924f5c39d
	// P2SH
	{
		...
		scripttype: rddscript.ScriptHashTy,
	},
	// Nulldata with no data at all.
	{
		...
		scripttype: rddscript.NullDataTy,
	},
	...
	// Nulldata with more than max allowed data (so therefore nonstandard)
	{
		...
		scripttype: rddscript.NonStandardTy,
	},
	...
	},
	{
		...
		script: []byte{
			...
			0x21, 0x1a, 0xb4, 0x14, 0xd5, 0x5a,
			rddscript.OP_TRUE,
			rddscript.OP_CHECKMULTISIGVERIFY,
		},
		scripttype: rddscript.NonStandardTy,
	},
	{
	...
	},
	{
		name: "doesn't parse",
		script: []byte{
			rddscript.OP_DATA_5, 0x1, 0x2, 0x3, 0x4,
		},
		scripttype: rddscript.NonStandardTy,
	},
}
...
func testScriptType(t *testing.T, test *scriptTypeTest) {
	scripttype := rddscript.GetScriptClass(test.script)
...
func TestIsPayToScriptHash(t *testing.T) {
	for _, test := range scriptTypeTests {
		shouldBe := (test.scripttype == rddscript.ScriptHashTy)
		p2sh := rddscript.IsPayToScriptHash(test.script)
		if p2sh != shouldBe {
...
func TestBadPC(t *testing.T) {
		...
		TxIn: []*rddwire.TxIn{
			{
				...
				SignatureScript: []uint8{rddscript.OP_NOP},
				Sequence:        4294967295,
			},
		},
		TxOut: []*rddwire.TxOut{
	...
	pkScript := []byte{rddscript.OP_NOP}

	for _, test := range pcTests {
		engine, err := rddscript.NewScript(tx.TxIn[0].SignatureScript,
			pkScript, 0, tx, 0)
		if err != nil {
...
func TestCheckErrorCondition(t *testing.T) {
	...
	engine, err := rddscript.NewScript(tx.TxIn[0].SignatureScript, pkScript,
		0, tx, 0)
	if err != nil {
		t.Errorf("failed to create script: %v", err)
	}
	...
	for i := 0; i < len(pkScript)-1; i++ {
		...
		if err != rddscript.ErrStackScriptUnfinished {
...
type TstSigScript struct {
	name               string
	inputs             []TstInput
	hashType           rddscript.SigHashType
	compress           bool
...
var coinbaseOutPoint = &rddwire.OutPoint{
	Index: (1 << 32) - 1,
}
...
var SigScriptTests = []TstSigScript{
	{
		...
		inputs: []TstInput{
			{
				txout:              rddwire.NewTxOut(coinbaseVal, uncompressedPkScript),
				sigscriptGenerates: true,
				inputValidates:     true,
				indexOutOfRange:    false,
			},
		},
		hashType:           rddscript.SigHashAll,
		compress:           false,
	...
	},
	{
		...
		inputs: []TstInput{
			...
			{
				txout:              rddwire.NewTxOut(coinbaseVal+fee, uncompressedPkScript),
				sigscriptGenerates: true,
				inputValidates:     true,
				indexOutOfRange:    false,
			},
	...
	},
	{
		...
		inputs: []TstInput{
			{
				txout:              rddwire.NewTxOut(coinbaseVal, compressedPkScript),
				sigscriptGenerates: true,
				inputValidates:     true,
				indexOutOfRange:    false,
			},
	...
	},
	{
		...
		inputs: []TstInput{
			...
			{
				txout:              rddwire.NewTxOut(coinbaseVal+fee, compressedPkScript),
				sigscriptGenerates: true,
				inputValidates:     true,
				indexOutOfRange:    false,
			},
	...
	},
	{
		...
		hashType:           rddscript.SigHashNone,
		compress:           false,
		scriptAtWrongIndex: false,
	},
	{
		...
		hashType:           rddscript.SigHashSingle,
		compress:           false,
		scriptAtWrongIndex: false,
	},
	{
		...
		hashType:           rddscript.SigHashAnyOneCanPay,
		compress:           false,
		scriptAtWrongIndex: false,
	},
	{
	...
	},
	{
		...
		inputs: []TstInput{
			{
				txout:              rddwire.NewTxOut(coinbaseVal, shortPkScript),
...
nexttest:
	for i := range SigScriptTests {
		...
		output := rddwire.NewTxOut(500, []byte{rddscript.OP_RETURN})
		tx.AddTxOut(output)

		for _ = range SigScriptTests[i].inputs {
			txin := rddwire.NewTxIn(coinbaseOutPoint, nil)
			tx.AddTxIn(txin)
		}
		...
		for j := range tx.TxIn {
			...
			script, err = rddscript.SignatureScript(tx, idx,
		...
		scriptFlags := rddscript.ScriptBip16 | rddscript.ScriptCanonicalSignatures
		for j, txin := range tx.TxIn {
			engine, err := rddscript.NewScript(txin.SignatureScript,
				SigScriptTests[i].inputs[j].txout.PkScript,
				j, tx, scriptFlags)
			if err != nil {
...
var classStringifyTests = []struct {
	name        string
	scriptclass rddscript.ScriptClass
	stringed    string
}{
	{
		name:        "nonstandardty",
		scriptclass: rddscript.NonStandardTy,
		stringed:    "nonstandard",
	},
	{
		name:        "pubkey",
		scriptclass: rddscript.PubKeyTy,
		stringed:    "pubkey",
	},
	{
		name:        "pubkeyhash",
		scriptclass: rddscript.PubKeyHashTy,
		stringed:    "pubkeyhash",
	},
	{
		name:        "scripthash",
		scriptclass: rddscript.ScriptHashTy,
		stringed:    "scripthash",
	},
	{
		name:        "multisigty",
		scriptclass: rddscript.MultiSigTy,
		stringed:    "multisig",
	},
	{
		name:        "nulldataty",
		scriptclass: rddscript.NullDataTy,
		stringed:    "nulldata",
	},
	{
		name:        "broken",
		scriptclass: rddscript.ScriptClass(255),
		stringed:    "Invalid",
	},
}
...
// bogusAddress implements the rddutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the rddutil.Address interface.
func (b *bogusAddress) IsForNet(net *rddnet.Params) bool {
	return true // why not?
}
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := rddutil.NewAddressPubKeyHash([]byte{
	...
	p2shMain, _ := rddutil.NewAddressScriptHashFromHash([]byte{
	...
	p2pkCompressedMain, err := rddutil.NewAddressPubKey([]byte{
		0x02, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
		0x69, 0xc2, 0xe7, 0x79, 0x01, 0x57, 0x3d, 0x8d, 0x79, 0x03,
		0xc3, 0xeb, 0xec, 0x3a, 0x95, 0x77, 0x24, 0x89, 0x5d, 0xca,
		0x52, 0xc6, 0xb4}, &rddnet.MainNetParams)
	if err != nil {
		t.Errorf("Unable to create pubkey address (compressed): %v",
			err)
		return
	}
	p2pkCompressed2Main, err := rddutil.NewAddressPubKey([]byte{
		...
		0xb1, 0x6e, 0x65}, &rddnet.MainNetParams)
	if err != nil {
	...
	p2pkUncompressedMain, err := rddutil.NewAddressPubKey([]byte{
		...
		0xf6, 0x56, 0xb4, 0x12, 0xa3}, &rddnet.MainNetParams)
	if err != nil {
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. uncompressed key.
		{
			...
			[]byte{
				...
				0xa3, rddscript.OP_CHECKSIG,
			},
			nil,
		},

		// Supported address types with nil pointers.
		{(*rddutil.AddressPubKeyHash)(nil), []byte{}, rddscript.ErrUnsupportedAddress},
		{(*rddutil.AddressScriptHash)(nil), []byte{}, rddscript.ErrUnsupportedAddress},
		{(*rddutil.AddressPubKey)(nil), []byte{}, rddscript.ErrUnsupportedAddress},

		// Unsupported address type.
		{&bogusAddress{}, []byte{}, rddscript.ErrUnsupportedAddress},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		pkScript, err := rddscript.PayToAddrScript(test.in)
		if err != test.err {
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*rddutil.AddressPubKey
		nrequired int
		expected  []byte
		err       error
	}{
		{
			[]*rddutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
			[]byte{
				...
				0x23, 0xc2, 0xc4, 0x09, 0x27, 0x3e, 0xb1, 0x6e,
				0x65,
				rddscript.OP_2, rddscript.OP_CHECKMULTISIG,
			},
			nil,
		},
		{
		...
		},
		{
			...
			1,
			[]byte{
				rddscript.OP_1, rddscript.OP_DATA_65,
				0x04, 0x11, 0xdb, 0x93, 0xe1, 0xdc, 0xdb, 0x8a,
				0x01, 0x6b, 0x49, 0x84, 0x0f, 0x8c, 0x53, 0xbc,
				0x1e, 0xb6, 0x8a, 0x38, 0x2e, 0x97, 0xb1, 0x48,
				0x2e, 0xca, 0xd7, 0xb1, 0x48, 0xa6, 0x90, 0x9a,
				...
				0x99, 0x9b, 0x86, 0x43, 0xf6, 0x56, 0xb4, 0x12,
				0xa3,
				rddscript.OP_1, rddscript.OP_CHECKMULTISIG,
			},
			nil,
		},
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		script, err := rddscript.MultiSigScript(test.keys,
			test.nrequired)
		if err != test.err {
...
func signAndCheck(msg string, tx *rddwire.MsgTx, idx int, pkScript []byte,
	hashType rddscript.SigHashType, kdb rddscript.KeyDB, sdb rddscript.ScriptDB,
	previousScript []byte) error {

	sigScript, err := rddscript.SignTxOutput(
		&rddnet.TestNet3Params, tx, idx, pkScript, hashType,
		kdb, sdb, []byte{})
	if err != nil {
...
func checkScripts(msg string, tx *rddwire.MsgTx, idx int,
	sigScript, pkScript []byte) error {
	engine, err := rddscript.NewScript(sigScript, pkScript, idx, tx,
		rddscript.ScriptBip16|
			rddscript.ScriptCanonicalSignatures)
	if err != nil {
...
func mkGetKey(keys map[string]addressToKey) rddscript.KeyDB {
	if keys == nil {
		return rddscript.KeyClosure(func(addr rddutil.Address) (*btcec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) rddscript.ScriptDB {
	if scripts == nil {
		return rddscript.ScriptClosure(func(addr rddutil.Address) (
			[]byte, error) {
			return nil, errors.New("nope")
		})
	}
	return rddscript.ScriptClosure(func(addr rddutil.Address) ([]byte,
		error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	hashTypes := []rddscript.SigHashType{
		rddscript.SigHashOld, // no longer used but should act like all
		rddscript.SigHashAll,
		rddscript.SigHashNone,
		rddscript.SigHashSingle,
		rddscript.SigHashAll | rddscript.SigHashAnyOneCanPay,
		rddscript.SigHashNone | rddscript.SigHashAnyOneCanPay,
		rddscript.SigHashSingle | rddscript.SigHashAnyOneCanPay,
	}
	tx := &rddwire.MsgTx{
		Version: 1,
		TxIn: []*rddwire.TxIn{
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address, err := rddutil.NewAddressPubKeyHash(
				rddutil.Hash160(pk), &rddnet.TestNet3Params)
			if err != nil {
			...
			pkScript, err := rddscript.PayToAddrScript(address)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			sigScript, err := rddscript.SignTxOutput(
				&rddnet.TestNet3Params, tx, i, pkScript,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*btcec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := rddutil.NewAddressPubKey(pk,
				&rddnet.TestNet3Params)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
				break
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := rddutil.NewAddressScriptHash(
				pkScript, &rddnet.TestNet3Params)
			if err != nil {
			...
			scriptPkScript, err := rddscript.PayToAddrScript(
				scriptAddr)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			sigScript, err := rddscript.SignTxOutput(
				&rddnet.TestNet3Params, tx, i, scriptPkScript,
				hashType, mkGetKey(map[string]addressToKey{
					address.EncodeAddress(): {key, false},
				}), mkGetScript(map[string][]byte{
					scriptAddr.EncodeAddress(): pkScript,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk1 := (*btcec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			address1, err := rddutil.NewAddressPubKey(pk1,
				&rddnet.TestNet3Params)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			pk2 := (*btcec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
			address2, err := rddutil.NewAddressPubKey(pk2,
				&rddnet.TestNet3Params)
			if err != nil {
			...
			pkScript, err := rddscript.MultiSigScript(
				[]*rddutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
...
func TestCalcMultiSigStats(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: rddscript.ErrStackShortScript,
		},
		{
			...
			expected: rddscript.ErrStackUnderflow,
		},
		{
			name: "multisig script",
			script: []uint8{
	...
	for i, test := range tests {
		if _, _, err := rddscript.CalcMultiSigStats(test.script); err != test.expected {
...
func TestHasCanonicalPushes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			script:   []byte{rddscript.OP_PUSHDATA1, 4, 1, 2, 3, 4},
			expected: false,
		},
	}

	for i, test := range tests {
		if rddscript.HasCanonicalPushes(test.script) != test.expected {
...
func TestIsPushOnlyScript(t *testing.T) {
	...
	if rddscript.IsPushOnlyScript(test.script) != test.expected {
